    ...
    m_minSignificance(0.1)
{
 ...
 SliceIdMonitoringTool::~SliceIdMonitoringTool() {
  ...
  void SliceIdMonitoringTool::SelectOutputPfos(const Algorithm *const pAlgorithm, const SliceHypotheses &nuSliceHypotheses, const SliceHypotheses &crSliceHypotheses, PfoList &selectedPfos, const SliceVector &sliceVector) {
    
  if (this->GetPandora().GetSettings()->ShouldDisplayAlgorithmInfo())
    std::cout << "----> Running Algorithm Tool: " << this->GetInstanceName() << ", " << this->GetType() <<std::endl;

  std::cout << "SlicePfo Info: "  << nuSliceHypotheses.size() << "  " << crSliceHypotheses.size() << "  " << selectedPfos.size() << "  " << sliceVector.size() << std::endl;
